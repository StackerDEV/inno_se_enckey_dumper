...
int main(int argc, char** argv)
{
	...
	*/
	...
	{
		SetConsoleTitle(L".:: Inno ROPS runtime encryption password extractor v1.0 ::.");
		system("Color 2");
		...
		if (strcmp(childexec, ".tmp") == 0 && strcmp(sL5.c_str(), "/SL5=") == 0) // verify arguments
		{
			...
			//std::wstring pathz = (WCHAR)lpCommandLine.c_str();
			if (CreateProcessW(NULL, (LPWSTR)lpCommandLine.c_str(), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
			{
				...
				*/
				char patVersion[] = {
					0x53, 0x00, 0x65, 0x00, 0x74, 0x00, 0x75, 0x00, 0x70, 0x00, 0x20, 0x00, 0x76, 0x00, 0x65, 0x00,
					0x72, 0x00, 0x73, 0x00, 0x69, 0x00, 0x6F, 0x00, 0x6E, 0x00, 0x3A, 0x00, 0x20, 0x00, 0x49, 0x00,
					0x6E, 0x00, 0x6E, 0x00, 0x6F, 0x00, 0x20, 0x00, 0x53, 0x00, 0x65, 0x00, 0x74, 0x00, 0x75, 0x00,
					0x70, 0x00, 0x20, 0x00, 0x76, 0x00, 0x65, 0x00, 0x72, 0x00, 0x73, 0x00, 0x69, 0x00, 0x6F, 0x00,
					0x6E, 0x00, 0x20
				};
				...
				if (VA)
				{
					...
					printf("\n");
					if (ReadProcessMemory(procInfoS2.hProcess, (LPVOID)VA, &vBuf, strlen(patVersion) + 12, &nSize))
					{
						int loc = 0;
	...
	{
		SetConsoleTitle(L".:: Inno main setup detour hook injector ::.");
		system("Color 4");
		printf("[+] Stage 1 staring main loader.\n");
		STARTUPINFO si = { sizeof(STARTUPINFO) };
		si.cb = sizeof(si);
		if (CreateProcess((LPCWSTR)argv[1], NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &procInfoS1))
		{
...
int injectHook(DWORD pid)
{
	...
	std::wstring dllpath = (LPWSTR)buffer;
	dllpath = dllpath.substr(0, dllpath.find_last_of(L"\\/")) + L"\\minHook.dll";
	const WCHAR* dll = dllpath.c_str();
	...
	HMODULE hModule = GetModuleHandle(L"kernel32.dll"); //Retrieves kernel32.dll module handle for getting loadlibrary base address
	...
	LPVOID lpSpace = (LPVOID)VirtualAllocEx(hProcess, NULL, sizeof(dll), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE); //Allocates space inside for inject.dll to our target process
	if (lpSpace == NULL)
	{
	...
	int n = WriteProcessMemory(hProcess, lpSpace, dll, sizeof(dll), NULL); // strlen to sizeof !!!!!
	if (n == 0)  //Write inject.dll to memory of process
	{
